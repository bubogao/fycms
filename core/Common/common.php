...
//新版分页
function get_new_page_eku($count,$currentPage,$totalpages,$halfPer=5,$url,$pagego)
{
	$ekupage = array();
	$ekupage['count']			=	$count;
	$ekupage['totalpages']	=	$totalpages;
	$ekupage['curpage']		=	$currentPage;
	$ekupage['firstlink']		=	str_replace('{!page!}',1,$url);//第一页链接
	$ekupage['prelink']		=	str_replace('{!page!}',$currentPage == 1 ? 1 : $currentPage - 1,$url);//上一页链接
	$ekupage['nextlink']		=	str_replace('{!page!}',$currentPage == $totalpages ? $totalpages : $currentPage + 1,$url);//下一页链接
	$ekupage['lastlink']		=	str_replace('{!page!}',$totalpages,$url);//最后一页链接
	for($i=$currentPage-$halfPer,$i>1 || $i=1,$j=$currentPage+$halfPer,$j<$totalpages||$j=$totalpages;$i<$j+1;$i++)
	{
		$arr = array();
		$arr['ii'] = $i;
		$arr['link'] = str_replace('{!page!}',$i,$url);
...
function get_actor_url($str,$num,$keyword,$classname){
	...
	foreach($arr as $key=>$val){
		...
		$restr .= '<a href="'.C('web_path').'index.php?s=video/search/actor/'.urlencode($val).'" target="_blank">'.$value.'</a> ';
		if(($key+1) == $num){
			break;
		}
	}
...
// 导演带链接
function get_director_url($str,$num,$keyword,$classname){
    $str = str_replace(' ','/',str_replace(',','/',$str));
	$arr = explode('/',$str);
	foreach($arr as $key=>$val){
		$value = $val;
		...
		$restr .= '<a href="'.C('web_path').'index.php?s=video/search/director/'.urlencode($val).'" target="_blank">'.$value.'</a> ';
		if(($key+1) == $num){
			break;
		}
	}
...
function getsearchurl($wd)
{
	...
	if(C('url_rewrite'))
	{
		...
		if (strpos('wd',$rewrite_videosearch) === false)
		{
			$showurl = str_replace(array('$page'),array($params['p'].'wd'),C('rewrite_videosearch'));
		}else{
			$showurl = str_replace(array('$page','$wd'),array($params['p'] ? $params['p'] : 1,$where['wd']),C('rewrite_videosearch'));
		}
		$url .= $showurl;
...
function getguestbookurl()
{
	if(C('url_rewrite'))
	{
		...
		$url = C('web_url');
		$url = '';
		$rewrite_videosearch = C('rewrite_guestbook');
		$showurl = str_replace(array('$page'),array($params['p'] ? $params['p'] : 1),C('rewrite_guestbook'));
		$url .= $showurl;
	}else{
		$url = C('web_url');
		$url .= 'index.php?s=Guestbook';
	}
	$url .= C('url_html_suffix');
	return $url;
}
		...
		if(C('url_rewrite')){
			...
			switch($mid)
			{
				...
				case 'special':
					...
					break;
				case 'map':
					$showurl = str_replace(array('$id','$limit'),array($arrurl['id'],$params['limit'] ? $params['limit'] : 500),C('rewrite_map'));
					break;
				default:break;
			}
...
function get_tag_gxcms($tag){
	...
	if('special'==$table){
		...
		foreach($list as $key=>$val){
			...
			$list[$key]['picurls'] = get_img_url($list[$key]['picurls'],$list[$key]['content']);
		}
	}	
	if ($table == 'comment')
	{
...
function getyearlistnew($style='on3',$count = 9,$cid,$year=NULL)
{
...
*/
require("./core/Common/con_common.php");
?>
