...
class SpecialAction extends HomeAction{
    ...
    public function lists(){
		...
		$ekupage = get_new_page_eku($count,$page,$totalpages,C('web_home_pagenum'),get_show_url('special',$jumpurl,2),false);
    ...
    public function detail(){
		...
		if($array){
			...
			$midsArr = explode(',',$array['mids']);
			$count = count($midsArr);
			$totalpages = ceil($count/25);
