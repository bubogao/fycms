	...
	* @返回赋值后的style数组,可在调用的类直接assign*/
    public function tags_style(){
		...
		if(C('url_html')){
			...
			$style['baidusitemap']  = get_show_url('map',array('id'=>'baidu','limit'=>500),1);
			$style['googlesitemap'] = get_show_url('map',array('id'=>'google','limit'=>500),1);
		}
		
		$this->add_tags_style($style);
		return $style;
    }	
	
	//影片统计
	private function add_tags_style(&$style)
	{
		$rs = D('video');
		$style['allMovieCount']			=	$rs->count('id');//影片总数
		$start = strtotime(date('Y-m-d 00:00:00'));
		$end = strtotime(date('Y-m-d 23:59:59'));
		$style['todayMovieCount']		=	$this->getCountData($rs,$start,$end);//影片今天更新总数
		
		$start = $start - 86400;
		$end = $end - 86400;
		$style['yestodayMovieCount']	=	$this->getCountData($rs,$start,$end);//影片昨天更新总数
		
		$start = strtotime(date("Y-m-d H:i:s",mktime(0, 0 , 0,date("m"),date("d")-date("w")+1,date("Y")))); 
		$end = $start*86400*7;
		$style['weekMovieCount']		=	$this->getCountData($rs,$start,$end);//影片本周更新总数
		...
		$style['monthMovieCount']		=	$this->getCountData($rs,$start,$end);//影片本月更新总数
		unset($rs);
	}
	
	private function getCountData(&$rs,$start,$end)
	{
		$where['addtime'] = array(between,array($start,$end));
		return $rs->where($where)->count('id');
	}
