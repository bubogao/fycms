...
class HtmlAction extends AdminAction{	
    ...
    public function cindex(){
		...
		$this->buildHtml("index",'./',C('temp_con_name').':index');
    ...
    public function maptop($id){
		...
		$this->buildHtml('top10','./'.C('url_dir_maps').'/','./template/'.C('default_theme').'/'.C('temp_con_name').'/top10.html');
		if (empty($id)) {
			$this->assign("jumpUrl",'?s=Admin/Html/Show/');
    ...
    public function videoshow(){
		...
		for ($ii = $start; $ii<=$end; $ii++) {
			...
			$pages .= get_cms_page_css($ii,$totalpages,C('web_home_pagenum'),$pageurl,false);
			$ekupage = get_new_page_eku($count,$ii,$totalpages,C('web_home_pagenum'),$pageurl,false);
			$channel['cid']  = $cid;
			$channel['page'] = $ii;
			$channel['count']= $count;
			$channel['pages']= $pages;
			$channel['ekupage']= $ekupage;
			if ($ii > 1) {
			...
			$this->buildHtml($listdir,'./',C('temp_con_name').':'.$channel['ctpl']);
	...
	public function createlist()
	{
		...
		*/
		for ($ii = $start; $ii<=$end; $ii++)
		{
			...
			$listdir = str_replace('{!page!}',$ii,get_show_url_dir('video',$cid,$ii));//目录路径
			$showurl = C('webpath').$listdir.C('html_file_suffix');//预览路径	
			$this->buildHtml($listdir,'./',C('temp_con_name').':'.$channel['ctpl']);			//print_r($channel);exit;
		}
			//exit($pages);
			//exit('wbf');
    ...
    public function createvideo($array){
		...
		$this->buildHtml($videodir,'./',C('temp_con_name').':'.'video_detail');//生成视频内容页
	...
	public function createdown($array)
	{
		...
		$this->buildHtml($videodir,'./',C('temp_con_name').':'.'video_down');//生成视频内容页
		$videourl = C('webpath').$videodir.C('html_file_suffix');//预览路径
    ...
    public function createplay($array,$ji,$s){
		...
		$this->buildHtml($playdir,'./',C('temp_con_name').':'.'video_play');//生成视频播放页
    ...
    public function createinfo($array){
		...
		$this->buildHtml($infodir,'./',C('temp_con_name').':'.'info_detail');
    ...
    public function specialshow(){
		...
		for ($ii=1; $ii<=$totalpages; $ii++) {	
			...
			$this->buildHtml($listdir,'./',C('temp_con_name').':'.'special_list');
    ...
    public function specialread(){
		...
		foreach($list as $key=>$value){
			...
			$this->buildHtml($specialdir,'./',C('temp_con_name').':'.'special_detail');
