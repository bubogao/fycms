 ...
 * @link    www.gxcms.com
 */
class CustomcollectinfoAction extends AdminAction{
	private $CModel;
	private $RModel;
	public function _initialize(){
		parent::_initialize();
	 	...
	 	$this->RModel =D('Admin.ColRuninfo');
	 }
	...
	public function Add() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			if($nid=$this->CModel->Save($Get)){//save
				$this->assign("jumpUrl",C('cms_admin').'?s=Admin/CustomCollectinfo/ListShow');
				//$this->assign("jumpUrl",C('cms_admin').'?s=Admin/CustomCollectinfo/ColRun/action/demo/type/all/nid/'.$nid);
				if(!empty($Get['nid'])) {
					$this->success('修改成功！');
		...
		switch($action){
			...
			case 'import': 
						   if(isset($_POST['submit'])){ 
					          ...
					          $this->assign("jumpUrl",C('cms_admin').'?s=Admin/CustomCollectinfo/Manage/action/import');
	...
	public function TestUrl() {
		...
		$this->assign('jumpUrl','?s=Admin/CustomCollectinfo/Add');
		$this->display('url_list_info');
	}
	...
	public function ColRun(){
		...
		$this->assign('jumpUrl','?s=Admin/CustomCollectinfo/ListShow');
	...
	public function AutoChannel(){
	  if(!empty($_GET['action'])){
        ...
        $this->assign("jumpUrl",C('cms_admin').'?s=Admin/CustomCollectinfo/AutoChannel');
        $this->success('操作成功!');
