...
class ConfigAction extends AdminAction{
...

	{
		...
		$old = array(
					...
					'rewrite_specialdetail'=>array('url'=>'/index.php?s=special/detail/id/$id.html'),

					'rewrite_guestbook'=>array('url'=>'/index.php?s=guestbook/lists/p/$page.html'),

					'rewrite_map'=>array('url'=>'/index.php?s=map/lists/limit/$limit/id/$id.html'),
					'rewrite_top10'=>array('url'=>'/index.php?s=Top10/lists/'),
...

		{
...

			{
				...
				$vm['url'] = str_replace($k4,'$'.$mm,$vm['url']);

				$mm++;

			}

			$v2 = str_replace(array('$id','$page','$wd','$limit'),array('(.*)','(.*)','(.*)','(.*)'),$config[$km]);
...

			{
				...
				$str .= "RewriteRule ^".substr($v2,1)."$ ".$vm['url']."\r\n";

			}else{

				$str .= "RewriteRule ^".$v2."$ ".$vm['url']."\r\n";

			}

			$strng .= "rewrite ^".$v2."$ ".$vm['url']."\tlast;\r\n";

			$strhttpd .= "RewriteRule ^".$v2."$ ".str_replace('?','\?',$vm['url'])."\r\n";

		}

		write_file('./.htaccess',$str);	

		write_file('./httpd.ini',$strhttpd);
